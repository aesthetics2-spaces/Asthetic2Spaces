...
const VendorAuthPage = () => {
  ...
  const handleToggleMode = (target) => setMode(target);

  const containerVariants = {
    hidden: { opacity: 0 },
    visible: { opacity: 1, transition: { duration: 0.8, ease: "easeOut" } },
  };
  const isLongForm = mode === "register"; 

  return (
    <div className="flex flex-col lg:flex-row bg-white flex-grow">

      {/* LEFT STYLISH SECTION (same as user auth) */}
      <motion.div
    ...
    ${isLongForm ? "min-h-full" : "h-screen"}
  `}
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute inset-0 opacity-5">
            <div
              ...
              style={{
                backgroundImage: `
                  linear-gradient(#E3B04B 1px, transparent 1px),
        ...
        <div className="relative z-10 max-w-2xl mx-auto text-center flex-1 flex flex-col justify-center">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <motion.p className="text-xl opacity-90 leading-relaxed">
              {mode === "forgot"
                ? "Enter your email to reset your password."
                : mode === "login"
                ? "Manage listings, track engagement, and reach more customers."
                : "Create your vendor account and start listing properties today!"}
            </motion.p>
          </motion.div>
        </div>
      </motion.div>
